...
│   ├── xai.py                      # TimeSeriesGradCAMRegression (집계 CAM)
│   ├── event_wraping.py            # DTW, distributional DTW, event weighting
│   ├── posthoc.py                  # matched-pair 분석, deletion test
│   ├── metrics.py                  # 시간축 기준 reference-based DTW 지표
│   ├── concepts.py                 # TCAVExtractorCV, C-DEW 분석
│   │
│   │  ── Explainability (new) ────────────────────────
...
│   ├── posthoc_analysis_v2.py      # legacy post-hoc 분석
│   ├── cdew_concepts_v2.py         # legacy C-DEW 개념 분석
│   ├── metrics_over_time_v2.py     # legacy DTW 지표 분석
│   ├── event_warping.py            # legacy event warping
│   ├── gradcam_tcav_analysis.py    # 신규: GradCAM + TCAV 분석 래퍼
│   └── run_analysis.py             # 신규: 전체 분석용 notebook 스타일 스크립트
│
├── tests/
